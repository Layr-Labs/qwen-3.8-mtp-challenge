...
# actually use ([A-Za-z0-9._/-]) it is. Outside that set it is not, and each
...
if [[ ! "${object_path}" =~ ${object_path_pattern} ]]; then
  echo "upload-r2-object: object path must match [A-Za-z0-9._/-]+ because the key bytes are signed and sent verbatim with no percent-encoding: ${object_path}" >&2
  exit 2
fi
...
service="s3"
# Hash the file's CONTENT through stdin, never `shasum -a 256 "${path}"`.
#
# perl's shasum (macOS ships it) escapes the FILENAME in its output and marks
# the line by prefixing the digest with a backslash whenever the name contains
...
#
# That 65-character non-hex value then lands in BOTH the signed canonical
# request and the x-amz-content-sha256 header, so it is self-consistent and
# still rejected -- R2 compares it against the body it received and answers
# 400/403. A path starting with '-' is worse: shasum parses it as options and
# dies ("Unknown option: d"). Reading from stdin removes the filename from the
# output entirely, so the digest is the content's and nothing else.
...
# against real R2 on the download twin, all six attempts carried the SAME
# x-amz-date. A SigV4 signature is only accepted inside R2's ~15 minute
# clock-skew window, so a run that spends long enough in retries turns a
# transient 503 into a permanent-looking 403 RequestTimeTooSkewed. curl cannot
# fix that: only the caller can re-sign. Nothing that varies per attempt may be
# hoisted out of this function.
sign_request() {
...
if [[ ! "${retry_delay_seconds}" =~ ${retry_delay_pattern} ]]; then
  echo "upload-r2-object: R2_RETRY_DELAY_SECONDS must be a non-negative integer" >&2
  exit 2
fi
...
#     -> upload_with_aws_cli returns non-zero and this script falls through
#     to the signed path SILENTLY.
#
...
if [[ "${R2_FORCE_SIGNED:-0}" != "1" ]] && upload_with_aws_cli; then
...
trap 'rm -f "${response_body_path}"' EXIT
curl_rc=0
http_status=""
attempt=1
while :; do
  # Truncate before every attempt: curl opens --output lazily, so an attempt
  # that dies at connect time or times out leaves the previous attempt's error
  # document in place to be misreported as this one's.
  : > "${response_body_path}"
  ...
  # nothing triggers neither a retry nor a timeout, so the job hangs to its
  # own limit with no diagnosis. --retry/--retry-all-errors/--retry-delay are
  # deliberately gone: a curl-level retry replays fixed argv and therefore
  # cannot re-sign. --location is deliberately gone too: on a cross-host
  # redirect curl DROPS the custom Authorization header but FORWARDS
  # x-amz-date and x-amz-content-sha256, so the follow-up arrives unsigned
  # with signing headers attached -- and for a PUT it would also re-send the
  # whole body somewhere it was never signed for. On a same-host redirect it
  # re-sends a signature bound to the OLD path. R2 path-style issues no
  ...
  echo "upload-r2-object: attempt ${attempt}/${max_attempts} failed (curl exit ${curl_rc}, HTTP ${http_status:-none}); re-signing and retrying" >&2
  attempt=$((attempt + 1))
