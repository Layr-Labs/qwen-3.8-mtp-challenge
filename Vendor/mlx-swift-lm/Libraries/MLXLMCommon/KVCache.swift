...
public class ArraysCache: BaseKVCache {
    ...
    /// Checkpoint LADDER for multi-draft verify forwards, written by
    /// `Qwen35GatedDeltaNet` alongside `rollbackState` when `nConfirmed > 0`:
    /// entry `i` is the `(conv_state, ssm_state)` after the confirmed prefix
    /// plus the first `i` draft rows. Entry 0 therefore mirrors
    /// `rollbackState`, and a partial acceptance of `a` of `d` drafts restores
    /// entry `a` -- exact state, no repair re-forward, at any depth.
    /// Cleared on accept; restored on reject.
    public var rollbackStates: [(MLXArray, MLXArray)]? = nil
