...
final class Qwen35MTPDecoderLayer: Module {
    ...
    func callAsFunction(
        ...
        // Backbone already fuses this residual+norm boundary. The head
        // layer was left on the eager pair. Same kernel, same bf16/5120
        // guard, same bf16-round-before-square argument — bit-identical
        // to `h = x + r; postAttentionLayerNorm(h)`. One launch per
        // proposed token. Proposal-only; target exactness unchanged.
