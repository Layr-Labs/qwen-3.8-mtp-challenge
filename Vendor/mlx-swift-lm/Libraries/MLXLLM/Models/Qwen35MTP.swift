...
final class Qwen35MTPDecoderLayer: Module {
    ...
    /// Same forward as `callAsFunction`, but returns the residual stream and
    /// the MLP branch separately so the caller can fuse the final `h + mlpOut`
    /// add with its own following RMSNorm. Values are unchanged: the sum is
    /// deferred to the caller's fused kernel (`qwen35FusedResidualRMSNorm`),
    /// which is already bit-identical to `norm(bf16(x + r))` at the
...
final class Qwen35MTPModule: Module {
    ...
    func callAsFunction(
        ...
        // 3/4. Run the decoder layer(s), then the pre-lm_head norm. For the
        // 1-layer bf16/5120 head the layer's final `h + mlpOut` add and the
        // module norm collapse into one `qwen35FusedResidualRMSNorm` launch —
        // the un-normed sum has no other consumer on this path, and the fused
        // kernel is the same shipped, value-identical residual+norm used at
        // the post-attention boundary. Unexpected geometry keeps the eager
        // `norm(h + mlp)` sequence.
        if layers.count == 1, fused.dtype == .bfloat16, fused.dim(-1) == 5120,
           norm.weight.dtype == .bfloat16
        {
            let parts = layers[0].outputParts(fused, mask: mask, cache: cache.first)
            if parts.residual.dtype == .bfloat16, parts.mlpOut.dtype == .bfloat16 {
    ...
    func lastHiddenWithKVOnlyHistory(
        ...
        if current.dtype == .bfloat16, current.dim(-1) == 5120,
           norm.weight.dtype == .bfloat16
        {
            let parts = layers[0].outputParts(current, mask: mask, cache: cache[0])
            if parts.residual.dtype == .bfloat16, parts.mlpOut.dtype == .bfloat16 {
                return qwen35FusedResidualRMSNorm(
                    x: parts.residual, r: parts.mlpOut,
                    weight: norm.weight, eps: norm.eps
