...
final class Qwen35MTPModule: Module {
    ...
    /// Affine-4/group-64 embedding rows dequantized inside the dual-RMSNorm
    /// concat kernel. Dense-embed concat (`#879`/`#930`) died; this path
    /// only fires for QuantizedEmbedding and never materializes BF16 rows.
    /// Isolated from `#941`'s extra fused top-32 rerank (3.32553 miss).
