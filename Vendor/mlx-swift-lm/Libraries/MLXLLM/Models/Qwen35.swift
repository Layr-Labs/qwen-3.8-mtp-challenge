...
// Stage one gives each 1,024-logit tile its own 256-thread group. The previous
// kernel launched one group for the entire 98,330-element compact vocabulary,
// leaving each thread to scan roughly a hundred logits serially before the
// reduction. Ninety-seven independent tiles expose the otherwise idle GPU;
// stage two reduces their 97 winners in one small dispatch.
private let qwen35DraftSelectStage1Kernel = MLXFast.metalKernel(
    name: "qwen_mtp_draft_select_stage1",
    inputNames: ["logits"],
    ...
    source: """
        ...
        uint simd_group = simdgroup_index_in_threadgroup;
        uint begin = tile * uint(TILE_SIZE);
        uint end = min(begin + uint(TILE_SIZE), uint(REAL_COUNT));
        float best_value = 0.0f;
        uint  best_id    = 0;
        bool  have       = false;

        for (uint index = begin + thread_id; index < end; index += TG_SIZE) {
        ...
        threadgroup float scratch_value[SIMD_GROUPS];
        threadgroup uint  scratch_id[SIMD_GROUPS];
        if (simd_lane == 0) {
...
private let qwen35DraftSelectStage2Kernel = MLXFast.metalKernel(
    name: "qwen_mtp_draft_select_stage2",
    ...
    source: """
        ...
        for (uint index = thread_id; index < PARTIAL_COUNT; index += TG_SIZE) {
            float value = float(partial_values[index]);
            uint id = uint(partial_ids[index]);
            bool take = !have || qwen_draft_better(
                value, id, best_value, best_id);
            if (take) { best_value = value; best_id = id; have = true; }
        }
        if (!have) {
            best_value = NAN;
            best_id = 0xFFFFFFFFu;
...
extension Qwen35TextModel: MTPCapable {
    ...
    public func draftTokenID(_ x: MLXArray) -> MLXArray {
        ...
        let padded = _compactDraftHead!(x)
        let tileSize = 1024
        let stage1Threads = 256
        let partialCount =
            (Self.compactDraftRealCount + tileSize - 1) / tileSize
        let partials = qwen35DraftSelectStage1Kernel(
            [padded.reshaped([Self.compactDraftPaddedCount])],
            template: [
                ("REAL_COUNT", Self.compactDraftRealCount),
                ("TILE_SIZE", tileSize),
                ("TG_SIZE", stage1Threads),
                ("SIMD_GROUPS", stage1Threads / 32),
            ],
            grid: (partialCount * stage1Threads, 1, 1),
            threadGroup: (stage1Threads, 1, 1),
            outputShapes: [[partialCount], [partialCount]],
            outputDTypes: [.float32, .uint32]
        )
        let stage2Threads = 128
        let outputs = qwen35DraftSelectStage2Kernel(
            partials,
            template: [
                ...
                ("TG_SIZE", stage2Threads),
                ("SIMD_GROUPS", stage2Threads / 32),
            ],
            grid: (stage2Threads, 1, 1),
            threadGroup: (stage2Threads, 1, 1),
            outputShapes: [[1, 1]],
            outputDTypes: [.int32]
        )
        return outputs[0]
