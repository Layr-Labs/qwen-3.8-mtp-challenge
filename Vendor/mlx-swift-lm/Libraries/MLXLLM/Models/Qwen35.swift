...
public enum Qwen35CustomQMV {
    ...
    /// ratio). M=2 is the pair kernel left on the library launcher;
    /// this change routes it through the same DIRECT_NIBBLES wide helper and
    /// the same chunk-sum table as M>=4. M=3 stays replica-without-table.
    ...
    ///
    /// This change adds M=2 to the table. Hidden prompts run narrower than
    /// the local width-7 mix, so the pair kernel left on MLX is the width
    /// the ranked box actually hits between serial M=1 and the already-hoisted
