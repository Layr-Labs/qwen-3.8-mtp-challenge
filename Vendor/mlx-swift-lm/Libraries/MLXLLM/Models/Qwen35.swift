...
/// ONE launch replacing the wide verify's per-layer out-chain pair —
/// `MLXFast.rmsNorm(out, weight, eps)` followed by the compiled fp32
/// SiLU-gate product — for S >= 2 on the one-wide-call path. The RMS
/// reduction reproduces the vendored `rms_single_row` kernel's arithmetic
/// EXACTLY: 32 lanes x 4 sequential reads per row, the same simd_sum tree,
/// `precise::rsqrt(sumsq / Dv + 1e-6f)`, and the same double rounding —
/// `InT(x * inv)` first, then `w * t` — that the two-launch chain performs
/// through device memory. The gate half reproduces the compiled post-norm
/// body's fp32 chain: `sigmoid` through MLX's own
/// `1 / (1 + exp(|g|))` sign-selected form, the fp32 product, one bf16
/// store. Per layer this removes one launch and one [B,S,Hv,Dv] bf16
/// round-trip; the values are bit-identical to the pair it replaces
/// (verified element-for-element in OpCost/fusion probe tests).
private let qwen35GDNOutChainKernel: MLXFast.MLXFastKernel = {
    ...
    let source = """
        ...
        // Each simdgroup owns one (row, head) pair: ROWS_PER_GROUP pairs per
        // threadgroup. The RMS reduction is then a pure simdgroup tree,
        // bit-identical to rms_single_row's single-simdgroup path.
        const uint idx = threadgroup_position_in_grid.y * ROWS_PER_GROUP + sg;
        const uint row = idx / Hv;
        const uint head = idx % Hv;

        // x, y: [B, S, Hv, Dv] contiguous. z may be strided (fused in-proj
        // carrier): index it through the provided stride array.
...
/// Apply the fused out-chain when the geometry matches the kernel's baked
/// constants; the stock two-launch chain otherwise. Same values either way.
private func qwen35GatedDeltaOutChain(
    ...
    // `z_strides` is the wrapper's auto-generated per-input stride array
    // (ensureRowContiguous: false) — the same convention the packed prework
    // kernel relies on. Eight (row, head) pairs per threadgroup keep the
    // SIMD lanes full; S * Hv is always a multiple of 8 at these shapes.
