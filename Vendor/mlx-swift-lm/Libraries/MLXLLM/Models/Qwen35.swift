...
final class Qwen35GatedDeltaNet: Module {
    ...
    private func processChunkStashingPrefix(
        ...
        // S >= 2 lower bound is hard (the kernel's conv-state copy reads only
        // qkv rows, which is wrong at S < nKeep); above 9 no verify exists.
        // S == 2 rides the same kernel as the wide path: the copy loop's
        // `row + NKeep < T` guard emits state rows only from qkv rows, and at
        // S == 2 the state is rows 1..3 of [state | x0 x1] — exactly the
        // kernel's three emitted rows, so the S=2 conv-state output is
        // identical to the stock `newConvState = convInput[0..., S-1...]`
        // slice. Per-row q/k/v, g and beta are row-local, so their bytes do
        // not change with S.
