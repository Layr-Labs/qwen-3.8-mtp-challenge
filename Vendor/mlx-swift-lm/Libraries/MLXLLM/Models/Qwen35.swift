...
final class Qwen35GatedDeltaNet: Module {
    ...
    func callAsFunction(
           ...
           let midKernel = qwen35GatedDeltaMidKernel
        {
            ...
            let convInput = concatenated([convState, qkv], axis: 1)
            let nKeep = convKernelSize - 1
            // Conv state after token 0 = rows 1..nKeep of [convState | x0 x1];
            // final conv state = the usual trailing nKeep rows.
            let midConvState = convInput[0..., 1 ..< (1 + nKeep)]
            let newConvState = convInput[0..., (convInput.dim(1) - nKeep)...]
            let convOut = silu(conv1d(convInput))
            ...
            let outputs = midKernel(
                ...
                threadGroup: (32, 4, 1),
                outputShapes: [[B, S, numVHeads, headVDim], state.shape, state.shape],
                outputDTypes: [dtype, .float32, .float32]
            )
            cache?.rollbackState = (midConvState, outputs[2])
            out = outputs[0]
...
final class Qwen35FusedMLP: Module, UnaryLayer {
    ...
    /// Drop any lazily built fused bank so the next decode-width call rebuilds
    /// it from the CURRENT projection modules. Called after the MTP head's
    /// drafting requantization swaps this MLP's linears.
    func resetFusedBanks() {
        _fbfW = nil
        _fqW = nil
        _fqS = nil
        _fqZ = nil
...
extension Qwen35TextModel: MTPCapable {
    ...
    /// See `Qwen35MTPModule.quantizeForDrafting`. Call once, outside every
    /// scored window (the session's warm does).
    public func prepareMTPHeadForDrafting() {
        mtp?.quantizeForDrafting()
    }
...
extension Qwen35Model: MTPCapable {
    ...
    /// See `Qwen35TextModel.prepareMTPHeadForDrafting`.
    public func prepareMTPHeadForDrafting() {
        languageModel.prepareMTPHeadForDrafting()
    }
