...
final class Qwen35SparseMoeBlock: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        let gateLogits = gate(x)

        let k = topK
        let kth = gateLogits.dim(-1) - k
        // argPartition is order-preserving under the monotonic softmax, so
        // selecting on the raw logits yields the same top-k indices.
        let inds = MLX.argPartition(gateLogits, kth: kth, axis: -1)[
            .ellipsis, (kth)...]
        var scores: MLXArray
        if normTopkProb {
            // renorm(softmax_all(top-k)) == softmax(top-k logits): the full
            // partition-function cancels, so skip the num_experts-wide softmax.
            let topLogits = MLX.takeAlong(gateLogits, inds, axis: -1)
            scores = MLX.softmax(topLogits, axis: -1, precise: true)
        } else {
            let gates = MLX.softmax(gateLogits, axis: -1, precise: true)
