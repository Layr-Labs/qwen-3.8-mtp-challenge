...
final class Qwen35FusedMLP: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        if x.dim(-2) <= 9, let (g, u) = fusedGateUp(x) {
            return qhLinear(downProj, silu(g) * u)
        }
...
//   MLX_QWEN_HUFF_SELFTEST=1 GPU bit-equality self-test on first two tensors,
//                            with a deliberate negative control
...
enum Qwen35HuffCodec {
    ...
    static func isEligible(rows: Int, rowBytes: Int) -> Bool {
        rows >= 1024 && rows % 8 == 0 && rowBytes % rowPeriodBytes == 0
...
enum Qwen35HuffRuntime {
    ...
    static let selfTestRequested: Bool =
        ProcessInfo.processInfo.environment["MLX_QWEN_HUFF_SELFTEST"] == "1"
