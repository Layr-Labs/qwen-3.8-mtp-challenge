...
/// K-only twin of `qwen35AttentionQKRMSRoPE` for the proposal-head history
/// path. `appendHistoryKV` never produces queries, so the Q+K kernel would
/// invent a dummy Q half. Same RMS / partial-RoPE arithmetic and the same
/// `[B, H, L, D]` write as the K side of the fused decode kernel.
private let qwen35AttentionKRMSRoPEKernel = MLXFast.metalKernel(
    ...
    source: """
        ...
        uint local_row = row;
        uint batch = local_row / (key_heads * sequence_length);
        uint head_sequence = local_row % (key_heads * sequence_length);
        uint head = head_sequence / sequence_length;
        uint sequence = head_sequence % sequence_length;
...
final class Qwen35Attention: Module {
    ...
    func appendHistoryKV(_ x: MLXArray, cache: any KVCache) {
        ...
        // History flushes are L=2 (accept fold) or L~511 (seed prime). The
        // decode fused Q+K kernel caps L<=32 because that is the verify
        // width; this K-only path must cover the priming width or it would
        // miss the expensive case. Same RMS+RoPE contract as the K half of
        // `qwen35AttentionQKRMSRoPE`. Fail closed onto eager kNorm+RoPE.
