...
/// Embedding dequant-gather fused into the dual pre-fc RMSNorm+concat
/// dispatch: ONE launch for what the eager path does as a quantized
/// embedding gather + affine dequantize, then the dual-norm-concat kernel.
///
/// The embedding half dequantizes the pinned affine-4/g64 embed_tokens rows
/// inline using exactly the arithmetic of MLX's `affine_dequantize` op
/// (byte-packed codes, low nibble first, `scale * d + bias` in float, bf16
/// store), so the values are bit-identical to
/// `embedTokens(nextTokenIds)` + the dual-norm-concat kernel when the guard
/// fires. The hidden half and the reduction tree are cloned verbatim from
/// `qwen35DualRMSNormConcat`. Proposal-side only.
