...
final class Qwen35Attention: Module {
    ...
    // RANKED RECEIPT. Submission `83f0b282` carried this elimination, together
    // with the discarded top-two target readout, against the promoted frontier
    // `8e83c6b3`. The official M5 pair measured -0.137 % candidate seconds per
    // token, faster on 7 of 8 hidden prompts, with every per-prompt mean draft
    // length identical to three decimals, which is the signature of a
    // bit-exact change. The published score of that run still fell, because it
    // drew the 1st percentile of 648 board runs on the runner-owned serial
    // baseline that forms the ranked numerator and that candidate code cannot
    // touch.
    //
