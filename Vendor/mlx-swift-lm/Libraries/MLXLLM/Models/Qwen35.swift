...
/// The committed MTP-history path has no query consumer.  Prepare only its K
/// rows in final cache layout, avoiding the standalone RMSNorm result,
/// transpose, and partial-RoPE materializations that would otherwise sit
/// between the packed K/V projection and `cache.update`.
private let qwen35AttentionKRMSRoPEKernel = MLXFast.metalKernel(
    ...
    source: """
        ...
        for (uint i = 0; i < n_reads; ++i) {
            ...
            if (element < axis_size) {
                ulong index = input_base + ulong(element) * input_axis_stride;
                bfloat rms_value = bfloat(float(k[index]) * inv_mean);
        ...
        if (thread_id < rotary_pairs / n_reads) {
            for (uint i = 0; i < n_reads; ++i) {
                ...
                float x1 = float(normalized[pair]);
                float x2 = float(normalized[pair + rotary_pairs]);
                k_out[output_base + ulong(pair)] =
                    bfloat(x1 * costheta - x2 * sintheta);
                k_out[output_base + ulong(pair + rotary_pairs)] =
                    bfloat(x1 * sintheta + x2 * costheta);
            }
        }
    """,
    ensureRowContiguous: false
...
/// Internal for exact Metal parity tests. Input is `[B,L,H,D]`; output is the
/// row-contiguous `[B,H,L,D]` tensor consumed by `KVCache.update`.
func qwen35AttentionKRMSRoPE(
    ...
    return qwen35AttentionKRMSRoPEKernel(
