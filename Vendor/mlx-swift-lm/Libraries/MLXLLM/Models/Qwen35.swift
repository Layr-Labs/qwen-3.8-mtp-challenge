...
final class Qwen35GatedDeltaNet: Module {
    ...
    // (the `_qkvW` pattern) so Module parameter reflection never sees them
    // at load time.
...
// MARK: - Fused residual + RMS norm (PR #250 / 0792c75, restored onto 12864bc)
//
// `MLXFAST_QWEN_MTP_FUSION=0` restores the two-op `x + r` then `RMSNorm`
// path bit-for-bit. The ranked sandbox strips `MLXFAST_*`, so the fused
// path is the scored default.
...
private let qwen35FusionEnabled: Bool =
    ProcessInfo.processInfo.environment["MLXFAST_QWEN_MTP_FUSION"] != "0"
...
func qwen35FusedResidualRMSNorm(
    ...
    if !qwen35FusionEnabled {
        let h = x + r
        return (h, MLXFast.rmsNorm(h, weight: weight, eps: eps))
    }
...
final class Qwen35DecoderLayer: Module {
    ...
    func callAsFunction(
        ...
        if qwen35FusionEnabled && x.dtype == .bfloat16 && r.dtype == .bfloat16
            && x.dim(-1) == 5120
        {
...
public class Qwen35TextModelInner: Module {
    ...
    func callAsFunction(
        ...
        if qwen35FusionEnabled && hiddenStates.dtype == .bfloat16
            && hiddenStates.dim(-1) == 5120
        {
...
/// Offline equivalence gate for the fused residual+RMSNorm kernel. Needs
/// no checkpoint: compares `qwen35FusedResidualRMSNorm` against eager
/// `h = x + r; MLXFast.rmsNorm(h, weight, eps)` on synthetic bf16 rows of
/// the live hidden width 5120. Returns (checked, mismatches, firstBadTrial).
/// Never called on a scored path.
public func qwen35VerifyFusedResidualRMSNorm(
    trials: Int = 64, seed: UInt64 = 7, rows: Int = 8
) -> (Int, Int, Int) {
    MLXRandom.seed(seed)
    var bad = 0
    var firstBad = -1
    ...
    let eps: Float = 1e-6
    for trial in 0 ..< trials {
        let x = MLXRandom.normal([rows, axis]).asType(.bfloat16)
        let r = MLXRandom.normal([rows, axis]).asType(.bfloat16)
        let w = MLXRandom.normal([axis]).asType(.bfloat16)
        let fused = qwen35FusedResidualRMSNorm(x: x, r: r, weight: w, eps: eps)
        let eagerH = x + r
        let eagerN = MLXFast.rmsNorm(eagerH, weight: w, eps: eps)
        eval(fused.residual, fused.normed, eagerH, eagerN)
        let hOK = MLX.all(MLX.equal(fused.residual, eagerH)).item(Bool.self)
        let nOK = MLX.all(MLX.equal(fused.normed, eagerN)).item(Bool.self)
        if !hOK || !nOK {
            bad += 1
            if firstBad < 0 { firstBad = trial }
        }
