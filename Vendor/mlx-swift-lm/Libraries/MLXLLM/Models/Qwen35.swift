...
final class Qwen35FusedMLP: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        // Verify widths (S = 3..5) ride the fused gate/up path too: one
        // N=34816 launch instead of two N=17408. Bit-exact per output row by
        // the same argument as the promoted in-proj fuse (rows run along N,
        // K-reduction order is launch-independent at the qmv widths), and the
        // strided g/u slices cost no copy: silu/mul are elementwise and the
        // mul's output is contiguous for downProj. The M=512 prefill stays on
        // the separate calls (S > 9), matching the promoted in-proj gate.
        // The bf16 head-side MLP (proposal-only) fuses the same way.
