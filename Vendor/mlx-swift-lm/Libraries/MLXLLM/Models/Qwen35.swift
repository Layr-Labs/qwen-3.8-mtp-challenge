...
/// Lazily packed concat-on-N quantized projection bank: one `quantizedMM`
/// launch in place of N. Rows (M) are independent and each output column's
/// dot product reads only its own weight columns, so on the `qmv` kernel
/// family the fused call is bit-exact with the separate calls — the same
/// argument the promoted attention-QKV fusion rests on.
///
/// M GATE (the qmm trap): the dispatch (`mlx/backend/metal/quantized.cpp`)
/// routes M above the small-M batch limit (10-12 for K=5120-class shapes) to
/// the `qmm` kernel, whose N-tiling does NOT reproduce separate calls
/// bit-for-bit (logits drift 1-2 bf16 ULP and flip drafts). Decode rounds
/// run M <= 8; prefill (512) returns nil and keeps the stock calls.
final class FusedQuantizedPack {
    /// Rows at or below this use the qmv family; 8 keeps margin under the
    /// 10-12 dispatch batch limit for K=5120-class shapes on all archs.
    static let maxRows = 8

    private var packed = false
    private var w: MLXArray?
    private var s: MLXArray?
    private var z: MLXArray?
    private var boundaries: [Int] = []
    private var groupSize = 0
    private var bits = 0
    private var mode = QuantizationMode.affine

    /// Returns per-projection slices of the fused output, or `nil` when the
    /// input is too wide (prefill) or the projections are not uniformly
    /// affine-quantized (bf16 MTP head). Never partially mutates state: the
    /// bank is built only when every precondition holds.
    func call(_ x: MLXArray, _ projections: [Linear]) -> [MLXArray]? {
        guard x.ndim >= 2 else { return nil }
        let rows = (0 ..< x.ndim - 1).reduce(1) { $0 * x.dim($1) }
        guard rows > 0, rows <= Self.maxRows else { return nil }

        if !packed {
            guard let first = projections.first as? QuantizedLinear else {
                return nil
            }
            let quantized = projections.compactMap { $0 as? QuantizedLinear }
            guard quantized.count == projections.count,
                quantized.dropFirst().allSatisfy({
                    $0.groupSize == first.groupSize && $0.bits == first.bits
                        && $0.mode == first.mode
                }),
                first.mode == .affine
            else { return nil }

            // Biases must be uniformly present or uniformly absent across the
            // bank; a mixed bank cannot be expressed as one quantizedMM.
            let hasBias = quantized.compactMap { $0.biases }
            let biases: MLXArray?
            if hasBias.count == quantized.count {
                biases = concatenated(hasBias, axis: 0).contiguous()
            } else if hasBias.isEmpty {
                biases = nil
            } else {
                return nil
            }

            w = concatenated(quantized.map { $0.weight }, axis: 0).contiguous()
            s = concatenated(quantized.map { $0.scales }, axis: 0).contiguous()
            z = biases
            boundaries = quantized.map { $0.shape.0 }
        ...
        guard let w, let s else { return nil }
        let y = quantizedMM(
        ...
        var start = 0
        var slices = [MLXArray]()
        slices.reserveCapacity(boundaries.count)
        for n in boundaries {
            let end = start + n
            slices.append(start == 0 && end == y.dim(y.ndim - 1) ? y : y[.ellipsis, start ..< end])
            start = end
        }
        return slices
    }
...
final class Qwen35GatedDeltaNet: Module {
    ...
    /// Packed [in_proj_qkv; in_proj_z; in_proj_b; in_proj_a] bank (M-gated).
    private let inProjPack = FusedQuantizedPack()
    ...
    private func processChunkStashingRow0(
        ...
        // WIDTH-GENERAL boundary ingredients: the WHOLE chunk's tensors, so a
        // later replay can rebuild the recurrent state after any committed
        // prefix, not just row 0. Every leaf is a value captured at stash
        // time (MLXArray buffers are immutable; a later in-place cache-slot
        // write replaces a reference, never the buffers), same discipline as
        // `snapshotRecurrent`.
    ...
    /// Rebuild the recurrent boundary after the first `prefixRows` rows of a
    /// fused chunk (`prefixRows == 1` is the post-PRIMARY boundary the K=1
    /// reject path used) from a stash recorded by
    ...
    ///
    /// BIT-EXACT AT EVERY PREFIX. `gatedDeltaUpdate` runs the same Metal kernel
    /// the fused call ran; the kernel's per-`t` body is a fixed sequence of
    /// fp32 operations on a register-resident state, so the state after
    /// iteration `t` depends only on `ssmPre` and rows `0 ... t` -- executing
    /// rows `0 ..< prefixRows` alone reproduces exactly the state the fused
    /// call held after its own row `prefixRows - 1` (`StT` is fp32, so the
    /// extra store/load round trip is lossless). This is the sequential
    /// recurrence's own determinism: a prefix of the iteration sequence gives
    /// that prefix's states, whatever the total `T` of the original call was.
    /// The convolution half is not recomputed at all -- the post-prefix window
    /// `[prefixRows, prefixRows + nKeep)` of the stashed `convInput` is the
    /// same three rows chunk-C would have computed, exact by slicing.
    func recomputeFusedPrimaryBoundary(cache: MambaCache, prefixRows: Int = 1) -> Bool {
        guard let stash = cache.fusedPrimaryBoundary else { return false }
        let p = Swift.max(1, prefixRows)
        let (_, boundarySsm) = gatedDeltaUpdate(
            ...
            dtBias: dtBias,
            state: stash.ssmPre,
            mask: stash.mask.map { $0[0..., 0 ..< p] }
        )
        let nKeep = convKernelSize - 1
        cache[0] = stash.convInput[0..., p ..< (p + nKeep)]
        cache[1] = boundarySsm
    ...
    func callAsFunction(
        ...
        // One packed launch for in_proj_qkv|z|b|a at decode widths (M<=8);
        // bit-exact on the qmv family per the FusedQuantizedPack argument.
        // Prefill and the bf16 MTP head keep the stock per-projection calls.
        var qkv: MLXArray
        let z: MLXArray
        let b: MLXArray
        let a: MLXArray
        if let packed = inProjPack.call(inputs, [inProjQKV, inProjZ, inProjB, inProjA]) {
            qkv = packed[0]
            z = packed[1].reshaped(B, S, numVHeads, headVDim)
            b = packed[2]
            a = packed[3]
        } else {
...
// MARK: - Dense MLP (fused gate|up)

/// Dense feed-forward identical to `Qwen3NextMLP` in weights and math, with
/// `gate_proj` and `up_proj` packed on N behind an M gate at decode widths —
/// one qmv launch instead of two per layer per forward. `down_proj` and the
/// `silu(gate) * up` arithmetic are untouched, so the only numeric surface is
/// the concat-on-N pack (bit-exact on the qmv family; see
/// `FusedQuantizedPack`). Defined here rather than edited into
/// `Qwen3Next.swift` because only this file is in the benchmark's editable
/// paths; the `@ModuleInfo` keys match the checkpoint's, so weight loading
/// and quantization apply unchanged.
final class Qwen35FusedMLP: Module, UnaryLayer {
    @ModuleInfo(key: "gate_proj") var gateProj: Linear
    @ModuleInfo(key: "down_proj") var downProj: Linear
    @ModuleInfo(key: "up_proj") var upProj: Linear

    private let gateUpPack = FusedQuantizedPack()
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        if let gu = gateUpPack.call(x, [gateProj, upProj]) {
            return downProj(silu(gu[0]) * gu[1])
        }
...
public class Qwen35TextModelInner: Module {
    ...
    func recomputeFusedPrimaryBoundary(cache: [KVCache?], prefixRows: Int = 1) -> Bool {
        ...
        for (i, layer) in layers.enumerated() where layer.isLinear {
            guard let mamba = cache[i] as? MambaCache,
                  layer.linearAttn?.recomputeFusedPrimaryBoundary(
                      cache: mamba, prefixRows: prefixRows) == true
            else { return false }
        }
        return true
    }
...
extension Qwen35TextModel: MTPCapable {
    ...
    /// See ``Qwen35TextModelInner/recomputeFusedPrimaryBoundary(cache:prefixRows:)``.
    public func recomputeFusedPrimaryBoundary(cache: [any KVCache], prefixRows: Int = 1) -> Bool {
        let cacheOpt: [KVCache?] = cache.map { Optional($0) }
        return model.recomputeFusedPrimaryBoundary(cache: cacheOpt, prefixRows: prefixRows)
    }
...
extension Qwen35Model: MTPCapable {
    ...
    public func recomputeFusedPrimaryBoundary(cache: [any KVCache], prefixRows: Int = 1) -> Bool {
        languageModel.recomputeFusedPrimaryBoundary(cache: cache, prefixRows: prefixRows)
    }
