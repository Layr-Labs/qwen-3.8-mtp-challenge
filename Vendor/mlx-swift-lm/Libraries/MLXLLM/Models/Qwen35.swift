...
public class Qwen35TextModelInner: Module {
    ...
    func callAsFunction(
        ...
        // MLX forbids asyncEval while a graph transformation is tracing. The
        // MTP session sets this process-local marker only around the first,
        // synchronous invocation of each compiled verify closure. Ordinary
        // eager forwards and cached compiled invocations never see it.
        let tracingCompiledMTP =
            getenv("DARKBLOOM_MTP_COMPILE_TRACE_ACTIVE") != nil
        let ladderActive =
            (inputs.dim(1) <= 9 || prefillLadder) && !tracingCompiledMTP
...
public class Qwen35TextModel: Module, LLMModel, KVCacheDimensionProvider {
    ...
    public init(_ args: Qwen35TextConfiguration) {
        // On the ranked 128 GiB host, the model is wired into Metal residency
        // after the input-independent warm. Once the live weights no longer
        // create command-buffer residency pressure, the stock 50 MiB byte
        // budget needlessly splits a layer. Select the verified layer-sized
        // budget before any MLX evaluation can cache the environment knob.
        // Explicit operator values and the kill switch always win; smaller
        // developer machines retain stock behavior.
        let environment = ProcessInfo.processInfo.environment
        if args.hiddenLayers >= 16,
           ProcessInfo.processInfo.physicalMemory >= (96 << 30),
           environment["DARKBLOOM_POST_WIRE_COMMAND_BUFFER"] != "0"
        {
