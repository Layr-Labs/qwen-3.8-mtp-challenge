...
/// Backbone quantized-embedding gather + first-layer RMSNorm for every
/// decode-width forward that still rides per-row qmv (S <= 9), including
/// the depth-0 S=1 step. Prefill S > 9 stays on `embedTokens` so the
/// qmm/split-k reduction order of the pinned baseline is preserved, the
/// same envelope as fused in-proj and fused MLP. Affine-4/group-64 dequant
/// matches MLX `affine_dequantize` (8 nibbles per uint32, low nibble first,
/// `bfloat(q)*scale+bias`). Residual base is the dequantized row; attn
/// input is the RMSNormed row — one launch instead of gather + dequant +
/// RMSNorm.
...
public class Qwen35TextModelInner: Module {
    ...
    func callAsFunction(
        ...
        if seqLen >= 1, seqLen <= 9, inputs.ndim == 2,
