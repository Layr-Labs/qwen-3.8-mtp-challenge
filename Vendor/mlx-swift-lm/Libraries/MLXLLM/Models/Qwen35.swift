...
// `dispatch_threads`, which counts the grid in THREADS. The shipped launcher
// retains exactly the leading `ceil(M/IPG)` working groups and omits only
// later groups that return before any read or write; every retained
// `threadgroup_position_in_grid` and simdgroup index is the same value the
// incumbent reads.
//
...
public enum Qwen35CustomQMV {
    ...
    public static let defaultRouteWitness =
        "e120_default_route/tiered_switch/onepass67/tight_grid"
...
extension Qwen35TextModel: MTPCapable {
    ...
    private func clusterCandidateIDs(_ x: MLXArray) -> MLXArray? {
        ...
        // then deleted by a whole-file overlay; restored here. The shortlist
        // half stays with the row-top32 kernels that superseded it.) One
        // dispatch replaces the merge-sort partition plus probe compaction
        // with the same ascending candidate set.
