...
extension Qwen35TextModel: MTPCapable {
    ...
    public func compiledHeadFlushStep(
        ...
        let step = compile(
            ...
            // The two branches have DIFFERENT output layouts, so the last
            // row is sliced per branch with constant bounds:
            //   - the K/V-only path returns ONLY the final row, [1,1,H] —
            //     its last (only) row is index 0, NOT width-1;
            //   - the full forward returns [1,S,H] — its final row is the
            //     host constant width-1.
            // The branch choice is shape-determined (hidden.dim(1) == width,
            // concrete at trace time under shapeless:false) and the shape is
            // constant per width, so the traced branch always matches every
            // replay. Slicing the K/V-only result at width-1 instead would
            // clamp out of bounds to an empty [1,0,H] and the caller's
            // draftTokenID would crash reshaping it.
            if let kvOnly = mtp.lastHiddenWithKVOnlyHistory(
                hidden: args[0], nextTokenIds: args[1],
                embedTokens: embed, cache: layerCache)
            {
                return [kvOnly[0..., 0 ..< 1, 0...]]
            }
            let full = mtp(
                hidden: args[0], nextTokenIds: args[1],
                embedTokens: embed, cache: layerCache)
            return [full[0..., (width - 1) ..< width, 0...]]
        }
        return { hidden, tokens in
