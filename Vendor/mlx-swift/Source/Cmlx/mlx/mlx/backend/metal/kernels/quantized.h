    ...
    if (out_vec_size >= 4096) {
      ...
      switch (ntg.x) {
        ...
        case 8:
          ...
          // cross-row study) — a register cliff, not work scaling.
          // Exact: these vector lanes carry INDEPENDENT input rows and are never
          // reduced across (simd_sum reduces along K WITHIN a row), so moving a
          // row from lane 3 of a four-wide vector to lane 0 of a two-wide one
          // cannot reorder its scalar chain. Template admits it: M in [3,9],
          // 8 % 3 == 2 (no one-row tail), IPG 3 inside the wide helper's [2,4].
          // Receipts: 85d5bca3 2.91143, 93ce739b-era yzxoi 2.92675.
