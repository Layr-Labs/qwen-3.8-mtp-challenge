...
// M=7 crosses a measured register-pressure cliff for vec<float, 4> on current
// Apple GPUs. Keep the exact wide helper and its per-component arithmetic, but
// cover the seven input rows as 3+2+2 instead of 4+3. The existing host launch
// still supplies seven x-groups; only the first three do work.
//
