    ...
    if (out_vec_size >= 4096) {
      ...
      switch (ntg.x) {
        ...
        case 7:
          ...
          return;
        case 8:
          // 4+4, not 3+3+2. Two balanced four-row panels reduce per-stream
          // address setup, metadata handling, and loop overhead vs the
          // three-panel shape. Eight divides cleanly into two groups of four.
          // Fresh receipts (welttowelt b35b120 +0.67% on DIRECT_NIBBLES tree;
          // SSHdotCodes 578535f +0.23% on Amal's affine-2 tree) outweigh the
          // earlier cross-row study that preferred IPG=3 at lower frontier scores.
          // Lanes carry INDEPENDENT rows; no scalar reordering across panels.
