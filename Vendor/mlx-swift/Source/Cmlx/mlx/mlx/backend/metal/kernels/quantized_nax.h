...
[[kernel]] void affine_qmm_t_nax(
  ...
  // For tile-compatible matrices, remap each pair of host M groups into the
  // two N halves of one 128x32 tile. Four SIMD groups still compute one 32x32
  // output tile each, so accumulator shape and K reduction order are
  // unchanged, while a dequantized weight tile is reused across twice as many
  // rows.
  if constexpr (
      group_size == 64 && bits == 4 && aligned_N && !batched &&
      BM == 64 && BK == 64 && BN == 64 && WM == 2 && WN == 2) {
    if (M >= 128 && M % 128 == 0 && K % 128 == 0 && N % 64 == 0) {
