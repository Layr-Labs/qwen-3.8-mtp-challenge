...
METAL_FUNC void qmm_t_nax_tgp_impl_pipelined(
  ...
  dispatch_bool(!is_unaligned_sm, [&](auto kAlignedM) {
    dispatch_bool(aligned_N || !is_unaligned_bn, [&](auto kAlignedN) {
      ...
      // leading barrier is needed; iteration 0's top-of-loop barrier
      // publishes these writes before the first mma reads them.
      if constexpr (kAlignedN.value) {
        loader_w.load_unsafe();
      } else {
        loader_w.load_safe(short2(BK, tgp_bn));
      ...
      int i = 0;
      for (; i < (K / BK) - 1; i++) {
        // ---- barrier first: it publishes chunk i's staging writes AND
        // retires iteration i-1's mma reads of buf[(i+1)&1] before this
        // iteration's loader overwrites that buffer.
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // ---- fused phase: advance the source cursor to chunk i+1, then
        // dequant it into buf[(i+1)&1] while the mma below reads buf[i&1].
        loader_w.next();
      ...
      // iteration; publish it, then run the mma sub-loop from
      // buf[(K/BK - 1) & 1].
      {
        threadgroup_barrier(mem_flags::mem_threadgroup);
