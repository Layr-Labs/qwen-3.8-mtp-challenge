...
METAL_FUNC void qmv_fast_crossrow_affine4_g64(
  ...
  // Widen weight-read reuse from two verify rows to four: one weight-tile read
  // per K-block feeds two float2 pairs (rows first_m..first_m+3). Uniform across
  // the simdgroup (M, first_m are threadgroup uniforms), so every branch and
  // simd_sum runs on the full lane set. Structure and arithmetic mirror the
  // proven two-row kernel exactly -- only a second pair is added.
  const bool has_1 = first_m + 1 < M;  // second row of pair 0
  const bool has_2 = first_m + 2 < M;  // first row of pair 1
  const bool has_3 = first_m + 3 < M;  // second row of pair 1
  ...
  for (int k = 0; k < in_vec_size; k += block_size) {
    ...
    if (has_2) {
      ...
      if (has_3) {
        ...
        const float sum3 = load_vector<T, float, values_per_thread, 4>(
