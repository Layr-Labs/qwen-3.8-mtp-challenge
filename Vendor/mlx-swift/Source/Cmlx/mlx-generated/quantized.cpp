...
[[kernel]] void affine_qmv_fast(
  ...
  if (!batched && group_size == 64 && bits == 4 && out_vec_size >= 1024) {
    if (out_vec_size >= 2048) {
      ...
      // the 3-4-row-sharing kernels: the arithmetic is the same crossrow
      // family (bit-exact with the pair kernel and the M=1 reference), and
      // whether the thinned grid wins or loses is M5-behavior this host
      // cannot resolve — the ranked run decides.
      switch (ntg.x) {
        case 2:
