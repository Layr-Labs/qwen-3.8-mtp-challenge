...
// A/B kill switch for the per-M crossrow QMV input-row schedule.
//
// `MLX_QMV_CROSSROW_SCHEDULE` selects the schedule table baked into the JIT
// Metal source below:
//
//   0  stock uniform-IPG table (M=4 (4), M=7 (4,3), M=8 (4,4))
//   1  M=7 -> (3,2,2) only; M=4 and M=8 untouched
//   2  M=4 -> (2,2), M=7 -> (3,2,2), M=8 -> (3,3,2)   [default when unset]
//
// unset / empty / anything else -> the `#ifndef`-guarded default in the source.
//
// WHY THIS IS SAFE AS A RUNTIME KNOB, AND WHY IT DOES NOT PERTURB DISPATCH.
// It does not touch the host-side kernel selection logic at all: the host
// (backend/metal/quantized.cpp) still picks `affine_qmv_fast` by the same
// shape/bits/group-size rules and still launches `grid.x == M` threadgroups.
// The only thing that moves is which threadgroup evaluates which contiguous run
// of input rows *inside* the kernel, which is a permutation of work and not of
// arithmetic -- see the exactness note on
// `qmv_fast_crossrow_affine4_g64_sched` below. All three levels are
// bit-identical; this is a performance switch, not a behaviour switch.
//
// It is read once, at first use, and the resulting source string is cached for
// process lifetime. `Device::get_library` memoises compiled libraries in an
// in-process `std::unordered_map` only (backend/metal/device.cpp:770-786) --
// there is no on-disk library cache keyed by kernel name -- so two processes
// launched with different values of this variable cannot contaminate each
// other, and a single process always compiles exactly one variant.
const char* qmv_crossrow_schedule_override() {
  const char* raw = std::getenv("MLX_QMV_CROSSROW_SCHEDULE");
  if (raw == nullptr || raw[0] == '\0' || raw[1] != '\0') {
    return "";
  }
  switch (raw[0]) {
    case '0':
      return "#define MLX_QMV_CROSSROW_SCHEDULE 0\n";
    case '1':
      return "#define MLX_QMV_CROSSROW_SCHEDULE 1\n";
    case '2':
      return "#define MLX_QMV_CROSSROW_SCHEDULE 2\n";
    default:
      return "";
  }
}
...
const char* quantized() {
  // Function-local static: thread-safe initialisation, static storage duration,
  // so the returned pointer stays valid for the process lifetime exactly as the
  // string literal it replaces did.
  static const std::string source =
      std::string(qmv_crossrow_schedule_override()) + R"preamble(
