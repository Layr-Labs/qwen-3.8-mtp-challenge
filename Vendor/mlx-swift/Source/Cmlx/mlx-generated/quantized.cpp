...
// otherwise fall through to qmv_fast_impl. The serial leg runs NO 2-bit
// matmuls at all — fused qkv 8192, o_proj 5120, mlp 17408/20480, and
// lm_head 248_320 are affine4. Dispatch is restricted to the proposal MLP's
// 17_408-row expansion and 5_120-row contraction, preserving the promoted
// generic path for the 98_336-row affine2 draft readout. The block loop
// requires input widths divisible by 512; both MLP shapes satisfy that guard.
// The per-element expression is the same
...
[[kernel]] void affine_qmv_fast(
  ...
  if (!batched && group_size == 64 && bits == 2 &&
    ...
    // M == 1 affine2 proposal MLP only. The target and the compact draft
    // readout stay on their promoted paths.
    if (ntg.x == 1) {
