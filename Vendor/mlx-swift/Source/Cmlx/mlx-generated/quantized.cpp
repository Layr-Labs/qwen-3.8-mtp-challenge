...
[[kernel]] void affine_qmv_fast(
  ...
  if (!batched && group_size == 64 && bits == 4 && out_vec_size >= 1024) {
    if (out_vec_size >= 4096) {
      ...
      // Mid-width (1024 <= N < 4096): K/V and the proposal KV pack. Route
      // M=3..9 through the same already-compiled DIRECT_NIBBLES `_m`
      // helpers the wide table uses. M=2 stays on the pair kernel — `_m`
      // statically asserts M>=3, and NA=2+nibble was measured negative.
      switch (ntg.x) {
        case 2:
          qmv_fast_crossrow_affine4_g64<T, 2>(
              w, scales, biases, x, y, in_vec_size, out_vec_size,
