...
METAL_FUNC void qmv_fast_singlerow_affine4_g64_wide(
  ...
  for (int k = 0; k < in_vec_size; k += block_size) {
    ...
    for (int r = 0; r < rows_per_simd; r++) {
      ...
      const device uint4* wv = reinterpret_cast<const device uint4*>(wb);
      const uint4 words = *wv;
      // Repack the 16-byte vector's uint16 words in order (little-endian
      // lane order matches the stock uint16 walk), arithmetic only.
      const uint32_t halves[4] = {words.x, words.y, words.z, words.w};
      for (int i = 0; i < 8; i++) {
        packed[r][i] = uint16_t(
