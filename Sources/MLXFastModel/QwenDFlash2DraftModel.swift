...
private enum QwenDFlash2Geometry {
    ...
    static let trainedBlockSize = 8
    static let trainedDraftCount = trainedBlockSize - 1
...
private final class QwenDFlash2Attention: Module {
    ...
    func callAsFunction(
        ...
        // The official DFlash2 layers use sliding attention. Retain the
        // upstream committed-context truncation before appending the complete
        // proposal block.
        ...
        // The released Qwen3.8 DFlash2 config is non-causal inside its
        // anchor-plus-seven proposal block. Every proposal row may attend to
        // the whole block; only committed context is historical. Committed
        // context was already truncated to the released sliding window above.
...
private final class QwenDFlash2CandidateSelector: Module {
    ...
    func select(
        ...
        logits: MLXArray,
        anchor: MLXArray,
        mapCandidateIds: (MLXArray) -> MLXArray
    ) -> MLXArray {
        let topK = QwenDFlash2Geometry.selectorTopK
        let kth = logits.dim(-1) - topK
        let compactCandidates = argPartition(logits, kth: kth, axis: -1)[
            .ellipsis, kth...]
        let unary = takeAlong(logits, compactCandidates, axis: -1)
        // Unary scores are gathered in the projection's compact ID space.
        // The learned predecessor/successor codebooks, and the returned draft
        // path, are indexed by the target tokenizer's full vocabulary.
        let candidates = mapCandidateIds(compactCandidates)
...
final class QwenDFlash2DraftModel: Module {
    ...
    private let mapDraftTokenIds: (MLXArray) -> MLXArray

    init(
        embed: @escaping (MLXArray) -> MLXArray,
        lmHead: @escaping (MLXArray) -> MLXArray,
        mapDraftTokenIds: @escaping (MLXArray) -> MLXArray
    ) {
        let g = QwenDFlash2Geometry.self
        ...
        self.embed = embed
        self.lmHead = lmHead
        self.mapDraftTokenIds = mapDraftTokenIds
        super.init()
    }
    ...
    static func load(
        ...
        let model = QwenDFlash2DraftModel(
            ...
            lmHead: { [target] hidden in target.applyDraftLMHead(hidden) },
            mapDraftTokenIds: { [target] ids in target.mapDraftTokenIds(ids) })
    ...
    func propose(
        ...
        precondition(
            draftCount >= 1
                && draftCount <= QwenDFlash2Geometry.trainedDraftCount)
        let tokens = MLXArray(
            [Int32(anchorToken)]
        ...
        return candidateSelector.select(
            ...
            mapCandidateIds: mapDraftTokenIds)
    }
}
