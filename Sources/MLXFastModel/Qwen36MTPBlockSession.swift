...
public final class Qwen36MTPBlockSession {
    ...
    /// The complete continuation carried between rounds. Keeping the token,
    /// top-2 readout, and lazy hidden row behind one optional makes their
    /// shared lifetime explicit and avoids three independent optional updates.
    ...
    private var pending: PendingContinuation?
    ...
    /// Scratch roots for the round's sole blocking eval. The cache-state prefix
    /// is rebuilt each round, but retaining this Array's storage avoids one
    /// allocation on every speculative verification.
    private var verificationEvalArrays: [MLXArray] = []
    private var draftTokenBuffer: [Int] = []
    ...
    public func begin(seedTokens: [Int]) throws -> Int {
        ...
        _ = seedLogits
        let initialHidden = hiddenRow(hidden, hidden.dim(1) - 1)
        let lastLogits = model.applyLMHead(initialHidden)
        ...
        pending = PendingContinuation(
            primary: readTail.0[0], top2IDs: readTail.0,
            top2Values: readTail.1, hidden: initialHidden)
    ...
    public func generateRound(depth: Int) throws -> Qwen36MTPRoundResult {
        guard began, let continuation = pending else { throw Qwen36MTPSessionError.notBegun }
        ...
        let primary = continuation.primary
        ...
        if depth == Qwen36MTPLimits.serialControlDepth || draftCount == 0 {
            ...
            let serialPendingHidden = hiddenRow(serialHidden, serialHidden.dim(1) - 1)
            ...
            pending = PendingContinuation(
                primary: readTail.0[0], top2IDs: readTail.0,
                top2Values: readTail.1, hidden: serialPendingHidden)
        ...
        verificationEvalArrays.removeAll(keepingCapacity: true)
        for layer in cache {
            verificationEvalArrays.append(contentsOf: layer.state)
        }
        verificationEvalArrays.reserveCapacity(
            verificationEvalArrays.count + 2 + draftIdArrays.count)
        verificationEvalArrays.append(top2IDs)
        verificationEvalArrays.append(top2Values)
        verificationEvalArrays.append(contentsOf: draftIdArrays)
        eval(verificationEvalArrays)
        if Self.traceRounds { tEvalDone = DispatchTime.now().uptimeNanoseconds }

        draftTokenBuffer.removeAll(keepingCapacity: true)
        draftTokenBuffer.reserveCapacity(draftIdArrays.count)
        for draftID in draftIdArrays {
            draftTokenBuffer.append(Int(draftID.item(Int32.self)))
        }
        let drafts = draftTokenBuffer
        ...
        if drafts.count == 1 {
            // Fixed-K=1 production needs one scalar comparison; stop-token
            // termination is handled after committing this sole draft.
            acceptedCount = verifyArgmax[0] == drafts[0] ? 1 : 0
        } else {
            ...
            acceptedCount = matchingPrefix
        }

        var perRowTop2Tokens: [[Int]]
        var perRowTop2Logits: [[Double]]
        if draftCount == 1 {
            // Fixed-K=1 production needs exactly one verify metadata row here;
            // initialize it directly instead of reserving and running a loop.
            perRowTop2Tokens = [Array(flatTop2IDs[0 ..< 2])]
            perRowTop2Logits = [Array(flatTop2Values[0 ..< 2])]
        } else {
            perRowTop2Tokens = []
            perRowTop2Logits = []
        ...
        if acceptedCount == drafts.count {
            ...
            pending = PendingContinuation(
                primary: verifyArgmax[drafts.count], top2IDs: ids,
                top2Values: values,
                hidden: hiddenRow(
            ...
            {
                pending = PendingContinuation(
                    primary: verifyArgmax[acceptedCount],
                    top2IDs: perRowTop2Tokens[acceptedCount],
                    top2Values: perRowTop2Logits[acceptedCount],
                    hidden: hiddenRow(
                ...
                let (repairLogits, repairHidden) = model.callWithHidden(
                    ...
                    cache: cache, nConfirmed: 0)
                let repairPendingHidden = hiddenRow(
                ...
                pending = PendingContinuation(
                    primary: ids[0], top2IDs: ids,
                    top2Values: values, hidden: repairPendingHidden)
                perRowTop2Tokens.append(ids)
                perRowTop2Logits.append(values)
            }
        }
    ...
    private static func restoreAfterPrefixReject(
        ...
        for entry in cache {
            if let arrays = entry as? ArraysCache {
                let saved = acceptedCount == 0
                    ? arrays.rollbackState!
                    : arrays.rollbackCheckpoints[acceptedCount]
                arrays[0] = saved.0
                arrays[1] = saved.1
                arrays.rollbackState = nil
                arrays.rollbackCheckpoints.removeAll(keepingCapacity: true)
