...
public final class Qwen36MTPBlockSession {
    ...
    private static func warmTargetLaterWindowSDPA(_ cache: [KVCache]) {
        ...
        var outs: [MLXArray] = []
        // qL={2,3} restored on this crown. `#914` REPLACE-dropped `#895`'s
        // {1,2,3,4,5} warm while landing island dead-work. The exactness
        // chunk still splits width-7/8/9 into qL=5 + qL={2,3,4}, so those
        // PSOs first-touch inside the scored window without this. Ranked
        // receipt: `#895` 3.31965 vs prior 3.31894 on the concat-write tree.
        for qL in [1, 2, 3, 4, 5] {
