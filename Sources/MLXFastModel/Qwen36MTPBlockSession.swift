...
public final class Qwen36MTPBlockSession {
    ...
    public func warmAllDepths(maxDepth: Int) throws {
        ...
        // One-time head requantization (fc + MLP to affine-4; attention stays
        // bf16). Runs here so the quantization work AND the quantized kernel
        // shapes are all paid outside the scored window, on both sides — the
        // head is resident on the serial control too, so the load shape stays
        // identical.
        model.prepareMTPHeadForDrafting()
    ...
    public func begin(seedTokens: [Int]) throws -> Int {
        ...
        let primaryArr = argMax(lastLogits, axis: -1)
        let tailPair = Self.topTwoLazy(lastRow(lastLogits))
        eval(cache.flatMap { $0.state } + [primaryArr, tailPair.0, tailPair.1,
                                           pendingHidden!, hidden])
        pendingPrimary = primaryArr.item(Int.self)
        pendingTop2 = Self.topTwoRead(tailPair)
    ...
    public func generateRound(depth: Int) throws -> Qwen36MTPRoundResult {
        ...
        if depth == Qwen36MTPLimits.serialControlDepth || draftCount == 0 {
            ...
            let primaryArr = argMax(
                serialLogits[0..., (serialLogits.dim(1) - 1) ..< serialLogits.dim(1), 0...],
                axis: -1)
            let tailPair = Self.topTwoLazy(lastRow(serialLogits))
            eval(cache.flatMap { $0.state }
                + [primaryArr, tailPair.0, tailPair.1])
            pendingPrimary = primaryArr.item(Int.self)
            let readTail = Self.topTwoRead(tailPair)
        ...
        var draftId = argMax(model.applyLMHead(draftHidden), axis: -1)
            .asType(.int32)
        ...
        for _ in 1 ..< draftCount {
            ...
            draftHidden = headHidden[
                0..., (headHidden.dim(1) - 1) ..< headHidden.dim(1), 0...]
            draftId = argMax(model.applyLMHead(draftHidden), axis: -1)
                .asType(.int32)
            draftIdArrays.append(draftId)
        }
        ...
        let verifyArgmaxArr = argMax(verifyLogits, axis: -1)
        var top2Lazy: [(MLXArray, MLXArray)] = []
        top2Lazy.reserveCapacity(draftCount + 1)
        for index in 0 ... draftCount {
            top2Lazy.append(Self.topTwoLazy(verifyLogits[0, index]))
        }
        var bundle: [MLXArray] = [verifyArgmaxArr]
        bundle.append(contentsOf: draftIdArrays)
        for pair in top2Lazy {
            bundle.append(pair.0)
            bundle.append(pair.1)
        }
        eval(cache.flatMap { $0.state } + bundle)

        let verifyArgmax = verifyArgmaxArr[0].asArray(Int32.self).map { Int($0) }
        ...
        for index in 0 ..< draftCount {
            let (ids, values) = Self.topTwoRead(top2Lazy[index])
        ...
        if acceptedCount == drafts.count {
            ...
            let (ids, values) = Self.topTwoRead(top2Lazy[drafts.count])
                ...
                cache, to: committedOffset)
            {
                ...
                let repairPrimaryArr = argMax(
                    repairLogits[
                        0..., (repairLogits.dim(1) - 1) ..< repairLogits.dim(1),
                        0...],
                    axis: -1)
                let tailPair = Self.topTwoLazy(lastRow(repairLogits))
                eval(cache.flatMap { $0.state }
                    + [repairPrimaryArr, tailPair.0, tailPair.1])
                pendingPrimary = repairPrimaryArr.item(Int.self)
                let (ids, values) = Self.topTwoRead(tailPair)
                pendingTop2 = (ids, values)
                perRowTop2Tokens.append(ids)
                perRowTop2Logits.append(values)
            }
