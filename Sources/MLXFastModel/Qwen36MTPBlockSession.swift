...
public final class Qwen36MTPBlockSession {
    ...
    public func warmAllDepths(maxDepth: Int) throws {
        ...
        // fold width is d+1: every value in 2...maxDepth+1 is legal. Warming
        // only width 2 left the declared BF16 precision-island matmul at width
        // 5 to compile inside the second scored round (a repeatable ~210 ms
        // block versus ~93 ms steady state). Exercise the exact live expression
        // at every legal fold width while this throwaway cache is still untimed.
