...
public final class Qwen36MTPBlockSession {
    ...
    /// the decode crosses (768, then 1024) and running the qL shapes the live
    /// path dispatches is input-independent warm work with no effect on any
    /// scored tensor. Ranked receipts for this helper: qL {1,4,5} @1024
    ...
    private static func warmTargetLaterWindowSDPA(_ warmCache: [any KVCache]) {
        ...
        // The KVCacheSimple backing store grows in 256-row chunks, so the
        // decode's prefix crosses kL=640, 768, 896, 1024 (each 256-chunk
        // boundary plus the midpoints). Compile each boundary; if the SDPA
        // kernel family is per-kL, the midpoints cover shapes the 256-row
        // boundaries miss; if it is bucketed, the extra evals are cheap
        // untimed no-ops.
        for targetKL in [640, 768, 896, 1024] {
            ...
            // The 512 -> 768 -> 1024 FA concat family is itself a compile the
            // decode path will hit (KVCacheSimple.step grows by 256-row
            // chunks).
