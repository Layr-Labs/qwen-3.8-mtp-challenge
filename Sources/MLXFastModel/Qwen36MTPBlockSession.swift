...
public final class Qwen36MTPBlockSession {
    ...
    public func begin(seedTokens: [Int]) throws -> Int {
        ...
        // E045: seed-script hint for the extended draft vocabulary. Set per
        // request from this request's own seed, so covered (ASCII-core)
        // prompts keep the promoted core scan path bit-for-bit.
        if let hintable = model as? Qwen36DraftVocabularyHintReceiver {
            hintable.draftVocabularyWideHint = seedTokens.contains {
                $0 >= 98_304 && $0 < 248_044
            }
        }
