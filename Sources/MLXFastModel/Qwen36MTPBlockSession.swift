...
public final class Qwen36MTPBlockSession {
    ...
    ///
    /// PROMOTED-Q4 LOWER BRACKET ARM: the declared group-64 4-bit head cuts
    /// the artifact payload from 849 MB to 239 MB. This isolated h = 0.06 arm
    /// intentionally tests the aggressive edge of the plausible scheduler
    /// range. It seeds at depth 7, retaining the same online acceptance EMA
    /// and marginal stop rule so hard prompts can back off. The official
    /// eight-prompt receipt is authoritative; no kernel, verify-width, EMA,
    /// cache, or head change is bundled with this calibration arm.
    private static let headStepCostRatio = 0.06
