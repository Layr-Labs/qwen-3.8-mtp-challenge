...
public final class Qwen36MTPBlockSession {
    ...
    public init(
        ...
        draftPolicy = { [weak self] offeredDepth, _ in
            ...
            if self.dflash2 != nil {
                // The released training block is anchor + seven masks.  The
                // same causal model also supports the track's eighth legal
                // mask without another weight pass; the target remains the
                // authority if that extrapolated final proposal is wrong.
                // Local pre-submit validation accepted all eight proposals
                // while preserving exact target rows.
                return Swift.min(offeredDepth, 8)
            }
    ...
    private func warmDFlash2Shapes(
        ...
        // Compile every proposal width the trusted parent can offer, including
        // the one-position extrapolation beyond DFlash2's released
        // anchor-plus-seven training block.
        for depth in 1 ... Swift.min(maxDepth, 8) {
        ...
        for width in 1 ... (Swift.min(maxDepth, 8) + 1) {
