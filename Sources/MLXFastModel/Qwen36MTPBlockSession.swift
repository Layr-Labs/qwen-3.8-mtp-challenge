...
public final class Qwen36MTPBlockSession {
    ...
    private func warmAllDepthShapes(maxDepth: Int) throws {
        ...
        for width in 1 ... (maxDepth + 1) {
            ...
            // `callWithHiddenAndNormed`. Warming only `callWithHidden` leaves
            // the published post-norm output to cold-JIT inside the first
            // timed verify — the 7b33621 failure mode. (Restored from
            // 111b757, receipted twice, lost to a stale-base overlay.)
            // Width 1 deliberately keeps the plain pair: the serial leg runs
            // the same build, and its warm must not improve.
