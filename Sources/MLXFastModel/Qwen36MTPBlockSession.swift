...
public final class Qwen36MTPBlockSession {
    ...
    public func generateRound(depth: Int) throws -> Qwen36MTPRoundResult {
        ...
        // Trim the eval root: the bundle's top-2 rows derive from
        // verifyLogits, whose graph already reaches every final cache state
        // (each cache's update op feeds the same forward's attention), so
        // listing cache.state again only adds materialization traversal that
        // the CPU readouts below never read. eval(bundle) covers it.
        eval(bundle)
