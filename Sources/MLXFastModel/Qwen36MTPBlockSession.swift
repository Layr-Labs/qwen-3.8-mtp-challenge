...
public final class Qwen36MTPBlockSession {
    ...
    private func warmAllDepthShapes(maxDepth: Int) throws {
        ...
        // ROUND-1 EXACT-SHAPE WARM. The width ladder above walks every width
        // on ONE shared, already-walked cache, so width w first compiles at
        // kL ~= 512 + (w-1) + accumulated trims rather than at the kL the
        // scored first round actually dispatches: width 5, nConfirmed 1,
        // EXACT kL 512, on a cache whose allocator state matches a begin-fresh
        // session. The phase trace priced that gap at ~28 ms of eval wall
        // inside the timed candidate window on the reference box (round 1
        // vs steady rounds at 25-58 us). Replicate the round-1 shape here on
        // a throwaway cache: same op sequence, zero tokens, nothing read —
        // the same contract as every warm above. Candidate-side only: the
        // depth-0 serial control never dispatches this shape.
        let roundOneCache = model.newCache(parameters: nil)
        let (roundOneSeedLogits, _) = model.callWithHidden(
            input: LMInput.Text(
                tokens: MLXArray(seedWarmTokens).reshaped([1, 512])),
            cache: roundOneCache, nConfirmed: 0)
        _ = roundOneSeedLogits
        let roundOneBlock = Array(repeating: 0, count: 5)
        let (roundOneLogits, _) = model.callWithHidden(
            input: LMInput.Text(
                tokens: MLXArray(roundOneBlock).reshaped([1, 5])),
            cache: roundOneCache, nConfirmed: 1)
        let (roundOneIDs, roundOneValues) = Self.linearTopTwoRows(roundOneLogits)
        eval(roundOneCache.flatMap { $0.state }
            + [roundOneIDs, roundOneValues])
