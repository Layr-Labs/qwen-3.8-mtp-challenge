...
public final class Qwen36MTPBlockSession {
    ...
    public func warmAllDepths(maxDepth: Int) throws {
        ...
        if headHistoryCache == nil {
            ...
            if fresh.count == 1, let simple = fresh[0] as? KVCacheSimple {
                fresh = [CompilableKVCache.promote(from: simple, maxLength: 2048)]
            }
    ...
    public func generateRound(depth: Int) throws -> Qwen36MTPRoundResult {
        ...
        if let existing = headHistoryCache {
            ...
            // maxLength 2048 bounds the ranked run: 512-row seed + 512 decode
            // tokens (MLXFAST_QWEN_MTP_DECODE_TOKENS) + 8 drafts = 1032
            // rows worst case, ~1000 rows of headroom. Smaller buffers
            // halve the fixed-buffer attention overhead of the promoted
            // cache (every head sdpa reads the full buffer).
            if fresh.count == 1, let simple = fresh[0] as? KVCacheSimple {
                fresh = [CompilableKVCache.promote(from: simple, maxLength: 2048)]
            }
            headHistoryCache = fresh
        ...
        if Self.headCompileEnabled, (1...9).contains(flushWidth) {
