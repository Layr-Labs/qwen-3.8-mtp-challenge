...
public final class Qwen36MTPBlockSession {
    ...
    /// forward, and the cross-row QMV pairing now serves every width a round
    /// can dispatch), so the cap prices the marginal HEAD step against its
    /// acceptance odds; 8 is the full trusted offer, reached only on a
    /// sustained full-acceptance streak, and the ladder collapses back to 1
    /// after any miss so a cold prompt still degrades to plain K=1.
