...
public final class Qwen36MTPBlockSession {
    ...
    /// acceptance odds. With the paired multi-row QMV now covering every
    /// verify width up to the trusted maximum (M in [2, 9]), the marginal
    /// cost of a deeper draft during a full-acceptance streak is one head
    /// step (~1.5 GB of reads) against a committed token's ~14 GB serial
    /// cost — a ~11% break-even acceptance probability — so the ladder is
    /// allowed to climb to the trusted maximum of 8. The collapse-to-1 on
    /// any miss is unchanged: cold prompts still degrade to plain K=1.
    private static let streakDepthCap = Qwen36MTPLimits.maxDepth
