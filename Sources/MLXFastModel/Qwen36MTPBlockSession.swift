...
public final class Qwen36MTPBlockSession {
    ...
    /// The draft index from which each further draft also adds a chunk-B row.
    /// Width is `depth + 2`, `attentionWithCacheUpdate` splits at row 5, so
    /// chunk B first exists at depth 4.
    private static let chunkBPriceKnee = 4

    /// Price of the chunk-B row a deep draft adds, in the same units as
    /// `headStepCostRatio` (fractions of one serial forward).
    ///
    /// 0.02 — the gentler of the two robust values, chosen because the
    /// OPERATING acceptance is not known and this run is the calibration.
    /// Re-implementing this exact rule and comparing its chosen depth against
    /// the true optimum under a verify cost `v(w) = 0.815 + k*max(0, w-5)`,
    /// for a TRUE k that may differ from the priced one (mean % off optimum
    /// over per-position acceptance 0.88 ... 0.98):
    ///
    ///     true k   price 0   price .02   price .04
    ///       0.01     0.30       0.00        0.13
    ///       0.02     0.56       0.06        0.00
    ///       0.03     0.98       0.29        0.04
    ///       0.04     1.49       0.62        0.18
    ///       0.06     2.97       1.71        0.90
    ///
    /// Pricing 0.04 beats pricing nothing at EVERY true k in that band, and the
    /// worst case if the true cost is far lower than assumed is 0.13%. Our own
    /// ranked receipt puts k at 0.03-0.05: capping depth 8 -> 7 measured
    /// +1.31% on `919318e1`, which is the size this simulation reproduces there.
    ///
    /// Why this shape and not a cap or a last-rung surcharge. A cap is blind to
    /// WHY a round is deep and cannot set five different depths for five
    /// prompts — measured directly: removing the width-wall floor moved
    /// `00142a44` -3.16% but `ea82dcb5` +3.89%, because one wanted more depth
    /// and the other less. A correct PRICE needs no cap: the rule then lands on
    /// each prompt's own optimum through its own `reach` (d=5 at q=0.90, 6 at
    /// 0.92, 7 at 0.94, 8 at q>=0.96), and its distance from optimum falls from
    /// 0.66-2.90% to 0.00-0.81%.
    /// Why not 0.04, which the table above also supports. Both values beat
    /// pricing nothing at every true k in the band, but they differ in how much
    /// depth they remove if the real operating acceptance is LOWER than the
    /// 0.88-0.98 the table sweeps. Across q = 0.80 ... 0.99, k = 0.02 removes
    /// 10 drafts in total and never touches q >= 0.94; k = 0.04 removes 19 and
    /// takes two drafts off at q = 0.86-0.92. If this pool actually runs near
    /// q ~ 0.85, the larger price degenerates into "shorten every draft", which
    /// is precisely the dead constant-h direction (0.15 -> 2.667, 0.14 -> 2.766).
    ///
    /// I cannot pin the operating point from public data: mean draft length
    /// cannot be inverted to an acceptance while the streak gate's floor of 5
    /// is also depressing it, and every uncapped tree on the board carries some
    /// second schedule change. But the RANKED RECEIPT reports
    /// `effective_mean_draft_len` per prompt, so this submission measures it:
    /// a drop of about one draft on the mid prompts with the deepest ones
    /// unmoved confirms the model, and a uniform collapse says the price is
    /// too high for this pool. Raise toward 0.04 only on that evidence.
    private static let chunkBRowPrice = 0.02
    ...
    private func costModelDepth(offeredDepth: Int) -> Int {
        ...
        while depth < cap {
            ...
            // Price the chunk-B row this draft adds, instead of only the
            // head step. A round is one verify plus `d` head steps, and the
            // verify is NOT flat in width: above five rows
            // `attentionWithCacheUpdate` splits the decode attention at row 5
            // (AttentionUtils.swift:125) and chunk B carries `w - 5` rows. So
            // from the draft that first takes width past 5 — width is
            // `depth + 2`, hence `depth >= 4` — each further draft costs one
            // head step PLUS one chunk-B row, and the shipped rule charges only
            // the head step.
            let hStep = depth >= Self.chunkBPriceKnee
                ? h + Self.chunkBRowPrice
                : h
