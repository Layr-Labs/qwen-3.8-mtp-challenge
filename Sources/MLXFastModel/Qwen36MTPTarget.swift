...
public protocol Qwen36MTPTarget: AnyObject {
    ...
    ///
    /// WIDTH-GENERAL: `prefixRows == 1` rebuilds the post-primary boundary (the
    /// K=1 reject case); `prefixRows == a + 1` rebuilds the boundary after a
    /// committed prefix of `a` accepted drafts, which is what a partial
    /// acceptance at depth K > 1 needs. See
    /// ``Qwen35GatedDeltaNet/recomputeFusedPrimaryBoundary(cache:prefixRows:)``
    /// for the exactness argument.
    func recomputeFusedPrimaryBoundary(cache: [any KVCache], prefixRows: Int) -> Bool
